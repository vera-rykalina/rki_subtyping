...
for i, row in df_prrt.iterrows():
    if row["Stanford_PRRT_Subtype"] == row["Comet_PRRT_Subtype"] :
        df_prrt.at[i, ["PRRT_Subtype_Decision"]] = row["Stanford_PRRT_Subtype"]
    elif row["Comet_PRRT_Subtype"] == "_Seq. nicht klassifizierbar":
        df_prrt.at[i, ["PRRT_Subtype_Decision"]] = "_Seq. nicht klassifizierbar"
    else:
        df_prrt.at[i, ["PRRT_Subtype_Decision"]] = "Manual"

for i, row in df_env.iterrows():
    if row["Rega_ENV_Subtype"][0] == row["Comet_ENV_Subtype"] and len(row["Rega_ENV_Subtype"]) < 3 :
        df_env.at[i, ["ENV_Subtype_Decision"]] = row["Comet_ENV_Subtype"]
    elif row["Comet_ENV_Subtype"] == "_Seq. nicht klassifizierbar":
        df_env.at[i, ["ENV_Subtype_Decision"]] = "_Seq. nicht klassifizierbar"
    else:
        df_env.at[i, ["ENV_Subtype_Decision"]] = "Manual"
    

for i, row in df_int.iterrows():
    if row["Stanford_INT_Subtype"] == row["Comet_INT_Subtype"] :
        df_int.at[i, ["INT_Subtype_Decision"]] = row["Stanford_INT_Subtype"]
    elif row["Comet_INT_Subtype"] == "_Seq. nicht klassifizierbar":
        df_int.at[i, ["INT_Subtype_Decision"]] = "_Seq. nicht klassifizierbar"
    else:
        df_int.at[i, ["INT_Subtype_Decision"]] = "Manual"

print(df_prrt[["SequenceName","Rega_PRRT_Subtype", "Stanford_PRRT_Subtype", "Comet_PRRT_Subtype", "PRRT_Subtype_Decision"]].head(50))

print(df_env[["SequenceName","Rega_ENV_Subtype", "Stanford_ENV_Subtype", "Comet_ENV_Subtype", "ENV_Subtype_Decision"]].head(50))

print(df_int[["SequenceName","Rega_INT_Subtype", "Stanford_INT_Subtype", "Comet_INT_Subtype", "INT_Subtype_Decision"]].head(50))
