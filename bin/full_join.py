...
df_full_env = pd.merge(df_tag_env, df_env, on = "SequenceName", how = "left")
...
if "ENV_Subsubtype" in df_full_env.columns:
    df_full_env["ENV_Subsubtype"] = df_full_env["ENV_Subsubtype"].fillna(df_full_env["ENV_Info"])
...
df_full_env["SeqLength"] = df_full_env["Sequence"].str.len()
...
df_full_env["Scount"] = df_full_env["SequenceName"].str.extract("(^\d+-\d+)_\w{2,4}_\d+_?\w+?$", expand=True)
...
df_full_env = df_full_env.dropna(subset=["Scount"])
...
col5 = df_full_env.pop("Scount")
df_full_env.insert(0, "Scount", col5)
...
df_full_env["Repeat"] = df_full_env["SequenceName"].str.extract("^\d+-\d+_\w{2,4}_\d+(repeat?\d{1})?$", expand=True)
